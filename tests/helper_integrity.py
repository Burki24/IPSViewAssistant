#!/usr/bin/env python3
"""Verify vendored helper files against the local helper manifest."""
...
if set(subscriptions) != set(entries):
    raise SystemExit(
        f"Helper subscription/manifest mismatch: subscriptions={sorted(subscriptions)}, manifest={sorted(entries)}"
    )
...
for name, subscription in sorted(subscriptions.items()):
    ...
    if match is None or match.group(1) != meta.get("version"):
        raise SystemExit(
            f"Version mismatch for {name}: source={match.group(1) if match else None}, manifest={meta.get('version')}"
        )

print(f"Vendored helper integrity verified ({len(entries)} helper).")
